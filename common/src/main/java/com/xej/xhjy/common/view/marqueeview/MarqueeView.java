 ...
 * @class MarqueeView
 * @author dazhi
 * @Createtime 2018/7/12 11:08
 * @description 新闻轮播view,可左右上下播出
 * @Revisetime
 * @Modifier
 */
public class MarqueeView extends ViewFlipper {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (typedArray.hasValue(R.styleable.MarqueeViewStyle_mvTextSize)) {
            ...
            textSize = GenalralUtils.px2sp(context, textSize);
        }
    ...
    private void startWithFixedWidth(String notice, @AnimRes int inAnimResId, @AnimRes int outAnimResID) {
        ...
        int width = GenalralUtils.px2dp(getContext(), getWidth());
        if (width == 0) {
...
//        int limit = width / textSize;
        ...
        if (noticeLength <= limit) {
            ...
            list.add(notice.substring(0,20)+"...");
//            int size = noticeLength / limit + (noticeLength % limit != 0 ? 1 : 0);
//            for (int i = 0; i < size; i++) {
//                int startIndex = i * limit;
//                int endIndex = ((i + 1) * limit >= noticeLength ? noticeLength : (i + 1) * limit);
//                list.add(notice.substring(startIndex, endIndex-14)+"...");
//            }
        }
    ...
    public void startWithList(List<String> notices, @AnimRes int inAnimResId, @AnimRes int outAnimResID) {
        if (GenalralUtils.isEmpty(notices)) return;
        setNotices(notices);
        postStart(inAnimResId, outAnimResID);
    }
