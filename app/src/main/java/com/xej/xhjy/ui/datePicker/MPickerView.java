package com.xej.xhjy.ui.datePicker;
...
public class MPickerView extends View {
    ...
    public MPickerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mPaintText.setTextSize(Util.dpToPx(context, 15));
        mPaintSelect.setColor(0x000000);
        mPaintNormal.setColor(0xa2a2a2);
        mPaintText.setColor(Color.parseColor("#000000"));
        mPaintLine.setColor(Color.parseColor("#dbdbdb"));
        mPaintLine.setStrokeWidth(Util.dpToPx(context, 0.5f));
    ...
    private void draw(Canvas canvas, int type, int position, Paint paint) {
        ...
        if (position == 0) {
            ...
            if (mData.get(mSelectPosition).length() == 4) {
                startX = mPaintSelect.measureText("0000") / 2 + x - width / 2 + Util.dpToPx(mContext, 2.0f) + mPaddingStart - mPaddingEnd;
            } else {
                startX = mPaintSelect.measureText("00") / 2 + x - width / 2 + Util.dpToPx(mContext, 2.0f) + mPaddingStart - mPaddingEnd;
            }
            ...
            canvas.drawLine(0, line + metricsInt.ascent - Util.dpToPx(mContext, 2.0f), mWidth, line + metricsInt.ascent - Util.dpToPx(mContext, 2.0f), mPaintLine);
            canvas.drawLine(0, line + metricsInt.descent + Util.dpToPx(mContext, 2.0f), mWidth, line + metricsInt.descent + Util.dpToPx(mContext, 2.0f), mPaintLine);
            canvas.drawLine(0, Util.dpToPx(mContext, 0.5f), mWidth, Util.dpToPx(mContext, 0.5f), mPaintLine);
            canvas.drawLine(0, mHeight - Util.dpToPx(mContext, 0.5f), mWidth, mHeight - Util.dpToPx(mContext, 0.5f), mPaintLine);
        }
    }
...
//                Log.i(TAG, "-mMoveDistance---" + mMoveDistance);
    ...
    public void setDefaultValue(String value, String type, String replace) {
        if (mData.size() > 0) {
    ...
    private String getStringToNumber( String type, String value, String replace) {
        switch (type) {
            case DateType.MONTH:
            case DateType.DAY:
