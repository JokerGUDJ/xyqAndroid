    ...
    public void loadBuddyAvatar(String account) {
        final UserInfo userInfo = NimUIKit.getUserInfoProvider().getUserInfo(account);
             if (userInfo!=null&&userInfo.getAvatar()!=null){
                 String  url= userInfo.getAvatar().contains(".jpg")?userInfo.getAvatar():ImUtils.HEAD_IMAG_URL+userInfo.getAvatar()+".jpg";
//                 LogUtils.dazhiLog("headUrl----->"+url);
                 doLoadImage( url , DEFAULT_AVATAR_RES_ID, DEFAULT_AVATAR_THUMB_SIZE);
             }else{
    ...
    public void loadTeamIconByTeam(final Team team) {
        if (team != null){
            if(team.getIcon()!=null){
                if("1".equals(team.getIcon())){
                    doLoadImage(null,R.drawable.nim_avatar_meet, DEFAULT_AVATAR_THUMB_SIZE);
                }else{
                    doLoadImage(null,R.drawable.nim_avatar_group, DEFAULT_AVATAR_THUMB_SIZE);
                }
            }else{
                doLoadImage(null,R.drawable.nim_avatar_group, DEFAULT_AVATAR_THUMB_SIZE);
            }
