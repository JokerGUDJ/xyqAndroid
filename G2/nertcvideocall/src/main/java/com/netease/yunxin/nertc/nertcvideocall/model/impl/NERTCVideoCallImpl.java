...
public class NERTCVideoCallImpl extends NERTCVideoCall {
    ...
    private VideoCallOptions option;
    ...
    private void initNERtc(){
        ...
        NERtcEx.getInstance().setAudioProfile(NERtcConstants.AudioProfile.STANDARD_EXTEND, NERtcConstants.AudioScenario.SPEECH);
        ...
        try {
            NERtcEx.getInstance().init(mContext, appKey, rtcCallback, option.getRtcOption());
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void setupRemoteView(NERtcVideoView videoRender, String uid) {
        ...
        for (Map.Entry<Long, String> entry : memberInfoMap.entrySet()) {
            if (TextUtils.equals(uid, entry.getValue())) {
                ...
                NERtcEx.getInstance().setupRemoteVideoCanvas(videoRender, entry.getKey());
                return;
            }
        }
    ...
    private void leaveRtcChannel(RequestCallback<Void> callback) {
        ...
        int rtcResult = -1;
        if (NERtcEx.getInstance() != null) {
            rtcResult = NERtcEx.getInstance().leaveChannel();
        }
